...
public class ItemDatas {
    ...
    public static ItemDatas map(Item item){
        ModelMapper modelMapper=new ModelMapper();
        ItemDatas itemsdata = modelMapper.map(item, ItemDatas.class); 
        itemsdata.setItemId(item.getId());
        itemsdata.setSku(item.getItemCode());
        itemsdata.setItemCategory(item.getCategory());
        itemsdata.setPurchaseRate(item.getCostRate());
        itemsdata.setPurchaseRateFormatted(AppHelper.toFormattedDecimal(item.getCostRate()));
