...
//        System.out.println("SpecimenData received "+specimenData.toString());
    ...
    public static List<SpecimenData> map(List<Specimen> specimens) {
        List<SpecimenData> ss = new ArrayList<>();
        for (Specimen sd : specimens) {
            SpecimenData d = SpecimenData.map(sd);
            ss.add(d);
        }
        return ss;
    }
