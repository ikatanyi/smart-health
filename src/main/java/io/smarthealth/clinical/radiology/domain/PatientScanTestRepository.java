...
public interface PatientScanTestRepository extends JpaRepository<PatientScanTest, Long>, JpaSpecificationExecutor<PatientScanTest>{
    ...
    @Query("SELECT d.radiologyTest.scanName as testName, d.patientScanRegister.requestedBy as practitioner, count(d.radiologyTest.scanName) AS count, SUM(d.testPrice) as totalPrice FROM PatientScanTest d WHERE d.createdOn BETWEEN :fromDate AND :toDate Group by d.patientScanRegister.requestedBy")
