    ...
    public void processData() {
        ...
        try {
            ...
            String fetchPatientData = "select firstName, middleName, lastName, patientEmail, patientPhone,patientIDNumber, " +
                    "patientIPOPNumber, patientDateOfirth from tbl_registered_patients";
            ...
            while (rs.next()) {
                ...
                data.setCurrentPatientNo("patientIPOPNumber");
        ...
        try {
            for (PatientData da : patients) {
                List<VisitData> vd = fetchPatientVisitData(da);
...
//            System.out.println("Done inserting lab results");
    ...
    //fetch visits
    private List<VisitData> fetchPatientVisitData(PatientData pd) {
        List<VisitData> vd = new ArrayList<>();
        String visitData = "SELECT VisitId, date_Created, visit_no FROM  visit_db WHERE PatientID = '" + pd.getPvEntityNo() + "'";
        try {
